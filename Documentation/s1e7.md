## Search click button with 1 network request
...
## Search without SearchBox and move search-box to posts/partials
...
{{request('search','')}}
if request 'search'  exits then show if not show '' on search input
